...
if dropboxOutput=$($1/dropboxUploader/dropbox_uploader.sh download $OTRDROPBOXPATH $inFile); then
	...
	/usr/bin/transmission-remote localhost --auth wasmitnetzen:3hfxpzf-z_uJd8AP3C -a "http://81.95.11.2/torrents/$date/$line.torrent"
	...
	echo "Dropbox Uploader failed: $dropboxOutput"
fi
